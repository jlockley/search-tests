...
public class TestCategoryCountCommas {
    ...
    public TestCategoryCountCommas(String searchInterface, String searchTerm, String searchTerm2) throws Exception{
    ...
    public static ArrayList<String> getFamilies(String searchTerm) {
    	families = SIMILAR_SEARCH.createFamilies("similar_search_commas", searchTerm, "category_count_families");
    	return families;
    }
    
    @Parameters(name= "{1} VS {2}")
    public static List<Object[]> getTestData(){
    	return SIMILAR_SEARCH.createTestData("#", "similar_search_commas", "category_count_data");
    }
    ...
    public void runSimilarSearchTests(){
    	...
    	calculateUniqueItems(result1, result2, commonFamilies, resultSize1, resultSize2);
		
    	outputResultObjects(resultSize1, resultSize2);

		//printUniqueItems();
    	
    	Assert.assertTrue(String.format("Expected 0 unique Items in Expected Families - search term: {%s} unique products from expected families: {%s}", searchTerm1, uniqueProductsOne.size()), uniqueProductsOne.size() == 0 );
    	Assert.assertTrue(String.format("Expected 0 unique Items in Expected Families - search term: {%s} unique products from expected families: {%s}", searchTerm2, uniqueProductsTwo.size()), uniqueProductsTwo.size() == 0 );
     }
    
    private void calculateUniqueItems(EndecaResult result1, EndecaResult result2, List<String> commonFamilies, Integer resultSize1, Integer resultSize2){
		...
		for (String family : commonFamilies){
		 List<TreeSet<EndecaItem>> comparedProducts = SimilarSearchTools.compareResults(family, result1, result2);
		 if(!comparedProducts.get(0).isEmpty()){
			 List<TreeSet<EndecaItem>> uniqueProducts =  SimilarSearchTools.removeProductsWithoutAttribute(comparedProducts, searchTerm1);
